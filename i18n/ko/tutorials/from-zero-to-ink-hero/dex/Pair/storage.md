...
The _./uniswap-v2/logics/Cargo.toml_ will be a `rlib` crate named `"uniswap_v2""` and import crates from ink!, scale, and Openbrush (with feature `"psp22"`)
...
_./uniswap-v2/logics/Cargo.toml_
...
Let's create a storage struct in _./logics/impls/pair/data.rs_. Name the struct `Data` and add all the required fields.
...
in the _./logics/traits/pair.rs_ file, let's create a trait with the getters functions and make them callable with `#[ink(message)]` :
...
in _./logics/impls/pair/pair.rs_ add an impl block for generic type `data::Data`. We wrap the Data struct in Storage trait to add it as trait bound.
...
In _./contracts/pair/Cargo.toml_ import the uniswap-v2 logics crate and add it to the std features
...
In the contract _lib.rs_ import everything from pair traits (and impls):
