...
For each H160 address, the EVM pallet uses `AddressMapping` to generate a corresponding SS58 address, which is the account that's _actually_ used to interact with the native side of chain, including by holding its balance in `pallet-balances`. So, the native balance is _actually_ stored in `pallet-balances` via the SS58 address generated by the mapping.
...
This is valid for all EVM addresses and _does not require AU to be enabled._
